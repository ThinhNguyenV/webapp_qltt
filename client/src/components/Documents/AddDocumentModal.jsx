...
export default function AddDocumentModal({ show, onClose, categories, tags, onSuccess }) {
  const { user } = useAuth();
  ...
  return (
    <Modal show={show} onClose={handleClose}>
      <div className="modal-header">
        <h2><i className="fa-solid fa-file-circle-plus" /> Thêm Tài Liệu Mới</h2>
      ...
      <form onSubmit={handleSave}>
        ...
        <div className="modal-footer">
          <button type="button" className="btn-ghost" onClick={handleClose}>Hủy</button>
          <button type="submit" className="btn-primary" disabled={saving}>
            {saving ? 'Đang lưu...' : <><i className="fa-solid fa-floppy-disk" /> Lưu Tài Liệu</>}
          </button>
        </div>
      </form>
    </Modal>
